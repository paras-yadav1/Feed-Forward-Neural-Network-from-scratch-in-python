...
class FNN(object):
    ...
    def forwardprop(self, data, labels=None):
        ...
        """
        # Instructions:
        # forward propogate the given data through the layers and get
        ...
        ####################
        # AND HERE: Use the output of last layer to complete right hand side of probs
        ####################
    ...
    def backprop(self, labels):
        ...
        """
        # Instructions
        # Note: The activations are stored in each layer's 'a' attribute
        # use d_loss (the derivative of loss) to compute the gradients at the last layer
        ####################
        # YOUR CODE HERE: complete the right hand size of d_outputs
        ####################
        d_outputs = np.zeros_like(labels)
        d_outputs = self.d_loss(self.layers[len(self.layers)-1].a , labels)
        # use Layer.backward to propogate the loss backwards
        # Note: You don't need to return anything.
        # The Layer.backward method stores d_w and d_b for later use by optimizer to update the actual parameters 
        ####################
        # YOUR CODE HERE
        ####################

        for i in reversed(range(len(self.layers))):
            d_outputs = self.layers[i].backward(d_outputs)
...
class Layer(object):
    def __init__(self, shape, activ_func):
        ...
        self.w = np.random.uniform(-np.sqrt(2.0 / shape[0]),np.sqrt(2.0 / shape[0]),size=shape)
    ...
    def forward(self, inputs):
        ...
        scores= np.add(np.dot(inputs, self.w) , self.b)
    ...
    def backward(self, d_outputs):
        ...
        self.d_w = (1/len(self.inputs)) * np.dot(self.inputs.T, d_scores)
        # d_inputs:
